    ...
    <div class="cultivation-info">
      <div class="realm-display">
        Cảnh giới hiện tại：<span
          class="realm-text"
          v-text="`${$levelNames(player.level)}(${player.reincarnation || 0} chuyển)`"
        />
      </div>
      <el-progress
...
export default {
    ...
    computed: {
        buttonsFor() {
            return [
                { text: 'Bắt đầu tu luyện', click: () => this.startCultivate(), disabled: !this.isStart },
                { text: 'Dừng tu luyện', click: () => this.stopCultivate(), disabled: !this.isStop },
                { text: 'Chuyển sinh đột phá', click: () => this.reincarnationBreakthrough() },
    ...
    methods: {
        startCultivate() {
            ...
            const timerId = setInterval(() => {
                if (this.player.cultivation <= this.player.maxCultivation) {
                    ...
                    this.texts.push(this.player.level < this.$maxLv ? 'Bạn bắt đầu thiền định, hấp thụ linh khí xung quanh. Tu vi đã tăng!' : 'Cảnh giới hiện tại của bạn đã viên mãn, cần chuyển sinh để tiếp tục tu luyện');
        ...
        triggerRandomEvent() {
            const randomEvents = [
                { type: 'resource', name: 'Linh thạch', amount: 100, description: 'Bạn phát hiện một đống linh thạch!' },
                { type: 'cultivation', name: 'Đốn ngộ', amount: 500, description: 'Bạn đột nhiên đốn ngộ, tu vi tăng vọt!' },
                { type: 'item', name: 'Đan dược', effect: 'Tăng 100 điểm tu vi', description: 'Bạn nhận được một viên đan dược quý giá!' },
                { type: 'skill', name: 'Kiếm pháp', effect: 'Tăng 10% công kích', description: 'Bạn lĩnh ngộ một môn kiếm pháp cao thâm!' },
                { type: 'lucky', name: 'Lôi kiếp', effect: 'Tu vi giảm 10%', description: 'Bạn gặp phải lôi kiếp!' },
            ];
        ...
        reincarnationBreakthrough() {
            ...
            if (this.player.level == this.$maxLv) {
                if (this.player.points) {
                    this.$notifys({ title: 'Chưa đủ điều kiện chuyển sinh', message: `Hiện còn ${this.player.points} điểm cảnh giới chưa sử dụng, không thể chuyển sinh` });
                    return;
                }
                if (this.player.taskNum >= reincarnation) {
                    const txt = this.player.reincarnation == 0 ? 'Thuộc tính kẻ địch sau chuyển sinh mạnh gấp trăm lần trước chuyển sinh<br>Trước khi chuyển sinh, hãy đảm bảo sức mạnh của bạn đủ để đánh bại đối thủ sau chuyển sinh, tránh bị kẹt tiến độ và phải xóa lưu trữ để chơi lại' : 'Hành động chuyển sinh không thể hoàn tác, bạn có chắc muốn chuyển sinh?';
                ...
                this.$notifys({
                    title: 'Chưa đủ điều kiện chuyển sinh',
                    message: `Cảnh giới cần đạt <span class="textColor">${this.$levelNames(this.$maxLv)}</span> để đủ điều kiện chuyển sinh`,
                    dangerouslyUseHTMLString: true
                });
            }
        },
